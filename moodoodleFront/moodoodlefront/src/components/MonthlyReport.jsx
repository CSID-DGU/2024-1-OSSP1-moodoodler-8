...
export default function MonthlyReport({
  ...
  const { mainColor, monthlyReport, monthTagList } = useMonthlyReport({ selectedDate });
  const { profile, getUserProfile } = useProfile();

  const handlePrevMonth = (selectedDate) => {
    const newDate = dayjs(selectedDate).subtract(1, 'month').endOf('month').format('YYYY-MM');
    setSelectedDate(newDate);
  };

  const handleNextMonth = (selectedDate) => {
    const newDate = dayjs(selectedDate).add(1, 'month').startOf('month').format('YYYY-MM');
    setSelectedDate(newDate);
  };
              ...
              <>
                ...
                <div className='flex flex-col w-[310px] h-[45px]'>
                  ...
                  <div className='text-center text-[13px] text-darkGray'>
                    ...
                    <br />
                    <span className={`text-[${mainColor.color}]`}>[{mainColor.id}]</span>이 {mainColor.value}%를
                    차지하고 있어요~
                  </div>
                </div>
              </>
