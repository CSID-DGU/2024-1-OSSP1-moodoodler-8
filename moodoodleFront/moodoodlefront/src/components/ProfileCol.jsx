  ...
  return (
    <div className="flex w-[342px] h-[273px] justify-center items-center rounded-[20px] bg-white shadow-componentShadow">
      <div className="flex flex-col h-[215px] justify-between items-center">
        ...
        <img src={`${profile.profile_image}`} alt="프로필 사진" className="w-[99px] h-[99px] rounded-full" />
        <div className="flex flex-row gap-[6px] items-center">
          <p className="text-center text-[16px] font-semibold">{profile.nickname}</p>
          <img src={`/assets/${profile.public === true ? 'unprivate' : 'private'}.svg`} alt="private" />
        </div>
        <p className="text-center text-[14px] font-light">{profile.description}</p>
      </div>
    </div>
  );
}
